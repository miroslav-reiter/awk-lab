...
## ✅ Odporúčané vysvetlenie vo videu

Pri každom príkaze je vhodné vysvetliť tri veci:

- odkiaľ prichádzajú dáta, napríklad súbor, log alebo výstup systémového príkazu,
- ktoré polia AWK číta, napríklad `$1`, `$4`, `$9`, `$NF`,
- čo robí podmienka, napríklad `NR > 1`, `$1 == "root"`, `$9 ~ /^5/` alebo `NF > 0 && $1 !~ /^#/`.

Takto študenti nevidia iba hotový príkaz, ale chápu aj jeho skladbu, bezpečnostný význam a obmedzenia.
